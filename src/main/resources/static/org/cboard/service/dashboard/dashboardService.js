...
cBoard.service('dashboardService', function ($http) {
    ...
    this.saveThis = function () {
        $http.post("dashboard/saveData", {json: angular.toJson(this.boardData)}).success(function () {
        });
    };

    this.save = function (bData) {
        $http.post("dashboard/saveData", {json: angular.toJson(bData)}).success(function () {
        });
    };

    this.get = function (callback) {
        var _this = this;
        $http.get("dashboard/getData").success(function (response) {
            if (!response) {
                response = {rows: []};
            }
